(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('myRetailProductsApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-myRetailProductsApp-error');
                var entityKey = httpResponse.headers('X-myRetailProductsApp-params');
                if (errorHeader) {
